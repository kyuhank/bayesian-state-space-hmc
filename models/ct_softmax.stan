...
//   - The hard fmax floor is replaced with a differentiable softplus floor.
//   - This variant is used as a sensitivity check against ct_fmax.
...
transformed parameters {
  ...
  // Higher values more closely approximate a hard floor while remaining differentiable.
  real smoothness = 50;
  ...
  for (t in 2 : (ntimes + 1)) {
    PtMedian[t] =
      log1p_exp(
        smoothness * (
          Pt[t - 1] + r * (1.0 - Pt[t - 1]) * Pt[t - 1] - Ct[t - 1] / K
        )
      ) / smoothness + 1e-4;
